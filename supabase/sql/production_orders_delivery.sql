...
--
-- AFTER RUN: wait a few seconds. If API still errors, open
--   Project Settings → API → scroll for schema reload / or redeploy edge.
--   This script ends with NOTIFY so PostgREST usually picks up changes immediately.
-- =============================================================================

-- (1) Customer columns (skip if you already have them)
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS customer_name text,
  ADD COLUMN IF NOT EXISTS customer_email text,
  ADD COLUMN IF NOT EXISTS customer_message text;

-- (2) Delivery token + audit columns + unique constraint + child tables + RPC
-- ---------------------------------------------------------------------------
...
-- Unique on token; PostgreSQL allows multiple NULLs (legacy rows without a token).
ALTER TABLE public.orders
...
-- ---------------------------------------------------------------------------
-- Master versions + revision requests
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Atomic download counter (service role RPC from app)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Ask PostgREST to reload schema cache (fixes PGRST204 after DDL)
-- ---------------------------------------------------------------------------
