...
export default async function DeliveryPage({ params }: PageProps) {
  try {
    ...
    if (!order?.delivery_access_token) {
      return invalidLink();
    }
    ...
    const label =
      (order.track_name ?? "").trim() || `Order ${orderId.slice(0, 8)}`;
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] text-gray-500 sm:text-sm">
                Order placed:{" "}
                <span className="font-medium text-black">{createdAt}</span>
              </p>
              <p className="mt-1 text-[13px] text-gray-500 sm:text-sm">
                Latest master:{" "}
    ...
    console.error("[delivery-page] fatal", {
      message: err.message,
      stack: err.stack,
    });
    throw err;
